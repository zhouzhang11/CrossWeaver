...
class BaseDataset_m(Dataset):
    ...
    def apply_shift(self, image, shift_x, shift_y):
        """
        Apply spatial shift to image with black padding.
        shift_x > 0: shift right
        shift_y > 0: shift down
        """
        h, w = image.shape[:2]
    ...
    def load_image_ir(self, i, rect_mode=True):
        ...
        if im is None:  # not cached in RAM
            ...
            # Apply shift before resize (shift is applied to original image)
            if self.shift_x != 0 or self.shift_y != 0:
                im = self.apply_shift(im, self.shift_x, self.shift_y)
