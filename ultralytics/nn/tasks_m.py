...
class DetectionModel_m(BaseModel_m):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        m = self.model[-1]
        if isinstance(m, (Detect, OBB)):  # includes OBB
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, \
                     ...
                     C2f_adalora_sym_m_homo, C2f_adalora_sym_m_het):
        ...
        # elif m in (Concat, ConcatFlex):
    # # 支持 tuple/list 的通道统计，递归展开后求和
    # def _sum_ch(v):
    #     if isinstance(v, (list, tuple)):
    #         return sum(_sum_ch(t) for t in v)
    #     return v
    # c2 = sum(_sum_ch(ch[x]) for x in f)
        elif m is Add:
            if type(f) == int:
                c2 = ch[f]
                args = [c2]
            ...
            c2 = ch[f[0]]
            args = [c2, args[1]]
        elif m is FourInputFusionBlock:
            c2 = ch[f[0]]
            args = [c2]
