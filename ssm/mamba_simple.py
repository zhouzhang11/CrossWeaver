    ...
    def forward(self, hidden_states, extra_emb=None, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v0":
                ...
                # if not self.if_devide_out:
                #     if self.use_norm:
                #         out = F.linear(self.norm(rearrange(y_e + y_f, "b d l -> b l d")),
                #                        self.out_proj.weight, self.out_proj.bias)
                #     else:
                #         out = F.linear(rearrange(y_e + y_f, "b d l -> b l d"),
                #                        self.out_proj.weight, self.out_proj.bias)
                # else:
                #     if self.use_norm:
                #         out = F.linear(self.norm(rearrange(y_e + y_f, "b d l -> b l d") / 4),
                #                        self.out_proj.weight, self.out_proj.bias)
                #     else:
                #         out = F.linear(rearrange(y_e + y_f, "b d l -> b l d") / 4,
                #                        self.out_proj.weight, self.out_proj.bias)
