...
# def local_reverse(x, w=5, H=20, W=20, flip=False, column_first=False):
#     """Local windowed scan in LocalMamba
#     Input: 
#         x: [B, C, L]
#         H, W: original width and height before padding
...
def local_scan(x, w=2, H=8, W=8, flip=False, column_first=False):
    ...
    Input:
        ...
        column_first: if True, traverse columns first then rows
        flip: if True, reverse the sequence at the end
    Return:
        y: [B, C, H*W]  (exactly the original element count, no more, no less)
    """
    B, L, C = x.shape
    assert L == H * W, "Input length must equal H*W"
    # Restore spatial shape
    x = x.view(B, H, W, C)

    # Determine block traversal order
    ys = []
    # First determine row/col block index sequences
...
def local_reverse(y, w=2, H=8, W=8, flip=False, column_first=False):
    """
    Reverse scan: restore local_scan output back to [B, C, H*W]
    Input:
        y: [B, C, H*W]
        other parameters same as local_scan
    Return:
        x_rec: [B, C, H*W]
    """
    B, C, L = y.shape
    assert L == H * W, "Input length must equal H*W"
    # Optionally reverse first
    if flip:
        y = y.flip(-1)

    # Write back block by block
